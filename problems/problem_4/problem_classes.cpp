...
public:
  Appliance() {}
  Appliance(std::string newName) {name = newName;}
  std::string getName() {return name;}

  virtual int getEnergyUsed(int time) { return time;}
  virtual int getEnergyUsed() {return 0;}
};
// Problem (2/4)
/******************************************************************************
...
public:
  Dishwasher(std::string newName) : Appliance(newName) {dishes = 0;}
  void loadDishes(int newDishes) {dishes = dishes + newDishes;}
  int getEnergyUsed() {return 500 + (dishes * 3);}
};
...
public:
  Washer(std::string newName) : Appliance(newName) {clothes = 0;}
  void loadClothes(int newClothes) {clothes = clothes + newClothes;}
  int getEnergyUsed() {return 200 + (clothes * 10);}
};
...
public:
  Microwave(std::string newName) : Appliance(newName) {food = 0;}
  void addFood(int newFood) {food = food + newFood;}
  int getEnergyUsed() {return 300 + food;}
};
